...
from app.services.r2_storage_service import PresignedUpload
...
def override_media_service() -> Iterator[None]:
    app.dependency_overrides[get_media_service] = FakeMediaService
    yield
...
async def test_create_media_returns_created_asset() -> None:
    ...
    assert response.json()["title"] == "Menu Board"
    ...
    async with AsyncClient(
        ...
        response = await client.post(
            ...
            json={
                ...
                "cdn_url": "https://cdn.example.com/media/11111111-1111-1111-1111-111111111111/v1/duplicate.png",
                "version": 1,
                "file_size": 2048,
                "content_type": "image/png",
            },
...
async def test_create_upload_url_returns_presigned_r2_contract() -> None:
    ...
    assert body["cdn_url"] == "https://cdn.example.com/media/11111111-1111-1111-1111-111111111111/v1/uploaded.png"
